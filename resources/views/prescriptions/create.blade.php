...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
            <h1 class="text-2xl font-bold text-gray-800 mb-6">Créer une Prescription pour la Consultation du
                {{ \Carbon\Carbon::parse($consultation->date_consultation)->format('d/m/Y') }}</h1>

            <form action="{{ route('consultations.prescriptions.store', $consultation) }}" method="POST">
                @csrf

                <div class="mb-6">
                    <label for="date_prescription" class="block text-gray-700 text-sm font-bold mb-2">Date de
                        Prescription</label>
                    <input type="date" id="date_prescription" name="date_prescription"
                        value="{{ old('date_prescription', now()->format('Y-m-d')) }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('date_prescription') border-red-500 @enderror">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function addMedicamentRow() {
                ...
                medicamentRow.classList.add('medicament-row', 'grid', 'grid-cols-12', 'gap-4', 'mb-4',
                    'items-center');
